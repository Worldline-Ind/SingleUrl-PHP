...
/*$ymlpath = __DIR__ . '/parameters.yml';
try {
    $data = yaml_parse_file($ymlpath);
    foreach ($data as $key => $params) {
        if (empty($params)) {
            throw new Exception("Parameters Value Can Not Be Blank Add Value Of $key", 1);
        }
    }
} catch (Throwable $e) {
    echo "<h3 style='color:#E21717'>" . $e->getMessage() . "</h3>";
    exit();
}*/
...
if ($_POST && isset($_POST['submit'])) {
    ...
    $transactionRequestBean->destinationVPA = $val['vpa'];
    ...
    $log  = "Name : ".$transactionRequestBean->customerName."; Date : ".date("F j, Y, g:i a")."; Request Data : ".$transactionRequestBean->merchantCode."|".$transactionRequestBean->ITC."|".$transactionRequestBean->customerName."|".$transactionRequestBean->requestType."|".$transactionRequestBean->merchantTxnRefNumber."|".$transactionRequestBean->amount."|".$transactionRequestBean->accountNo."|".$transactionRequestBean->destinationVPA."|".$transactionRequestBean->currencyCode."|".$transactionRequestBean->returnURL."|".$transactionRequestBean->shoppingCartDetails."|".$transactionRequestBean->TPSLTxnID."|".$transactionRequestBean->mobileNumber."|".$transactionRequestBean->txnDate."|".$transactionRequestBean->bankCode."|".$transactionRequestBean->email."|".$transactionRequestBean->key."|".$transactionRequestBean->iv."|".$transactionRequestBean->custId."|".$transactionRequestBean->webServiceLocator.PHP_EOL;
            ...
            <form method="post">
                <table class="table table-bordered table-hover">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" value="TWU" name="reqType" readonly>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td><label><span style="color:red">*</span> Account Number</label></td>
                        <td><input type="text" name="accNo" required /></td>
                    </tr>
                    <tr>
                        <td><label><span style="color:red">*</span> Destination VPA</label></td>
                        <td><input type="text" name="vpa" value="test@testbank" required /></td>
                    </tr>
